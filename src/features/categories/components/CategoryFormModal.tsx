...
export const CategoryFormModal: React.FC<CategoryFormModalProps> = ({
  ...
  const handleCreateLabel = async () => {
    ...
    try {
      ...
      alert(err.response?.data?.error || err.message || 'Lỗi khi thêm nhãn đối tượng');
    }
  };
  ...
  const confirmDeleteLabel = async () => {
    ...
    try {
      ...
      alert(err.response?.data?.error || err.message || 'Lỗi khi xóa nhãn đối tượng');
    } finally {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      alert(err.response?.data?.error || err.message || 'Lỗi khi sửa nhãn đối tượng');
    } finally {
          ...
          {editingLabel && (
            <div className="p-3 bg-amber-50/80 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-xl space-y-2 text-xs">
              <div className="font-bold text-amber-700 dark:text-amber-300 flex items-center gap-1">
                ...
                Sửa Nhãn Đối Tượng: <span className="text-amber-600 dark:text-amber-400">"{editingLabel.name}"</span>
              </div>
              <div className="flex items-center gap-2">
      ...
      {deletingLabel && (
        ...
        >
          <div className="py-2 text-sm text-slate-700 dark:text-slate-300 space-y-2">
            <p>
              Bạn có chắc chắn muốn <strong className="text-rose-600 dark:text-rose-400">xóa vĩnh viễn</strong> nhãn đối tượng <strong className="text-rose-600 dark:text-rose-400">"{deletingLabel.name}"</strong> khỏi cơ sở dữ liệu?
            </p>
